...
/** After first CdCheck2 patch, byte scan no longer matches ù reuse RVA on AuthServ reload (same module may stay patched in memory). */
static DWORD s_CdCheck2CallAddrRva = (DWORD)-1;
/** SecServ has multiple copies of the CdCheck2 data-check helper; after patching the first, scan hits the wrong one ù pin RVA. */
