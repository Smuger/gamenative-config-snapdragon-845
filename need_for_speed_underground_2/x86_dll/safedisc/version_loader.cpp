...
HANDLE CheckForSecDrvW(LPCWSTR lpFileName, CreateFileW_typedef CreateFileOrig)
{
	HANDLE ret = INVALID_HANDLE_VALUE;

	if (lpFileName)
	{
		if (!_wcsicmp(lpFileName, L"\\\\.\\Secdrv") || !_wcsicmp(lpFileName, L"\\\\.\\Global\\SecDrv"))
		{
...
/** SafeDisc probes locale DLLs (e.g. SD0409.dll) by bare name; retail layout is usually disc rootóredirect CreateFile to exe dir (optional) then CDROMDriveLetter root from version.json. */
...
HANDLE WINAPI CreateFileW_Hook(LPCWSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, HANDLE hTemplateFile)
{
	...
	{
		if (logCreateFile && usePath && wcscmp(usePath, L"CONOUT$") != 0)
			log("CreateFileW_Hook - lpFileName: %ls ret: %08X (SecDrv)\n", usePath, (unsigned)(uintptr_t)ret);
		return ret;
	}
	...
	ret = CreateFileW_Orig(usePath, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile);
	if (logCreateFile && usePath && wcscmp(usePath, L"CONOUT$") != 0)
		log("CreateFileW_Hook - lpFileName: %ls ret: %08X\n", usePath, (unsigned)(uintptr_t)ret);
	return ret;
}
...
HANDLE WINAPI CreateFileW_Hook_KBase(LPCWSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition, DWORD dwFlagsAndAttributes, HANDLE hTemplateFile)
{
	...
	{
		if (logCreateFile && usePath && wcscmp(usePath, L"CONOUT$") != 0)
			log("CreateFileW_Hook_KBase - lpFileName: %ls ret: %08X (SecDrv)\n", usePath, (unsigned)(uintptr_t)ret);
		return ret;
	}
	...
	ret = CreateFileW_Orig_KBase(usePath, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile);
	if (logCreateFile && usePath && wcscmp(usePath, L"CONOUT$") != 0)
		log("CreateFileW_Hook_KBase - lpFileName: %ls ret: %08X\n", usePath, (unsigned)(uintptr_t)ret);
	return ret;
}
...
/** SafeDisc unpack dirs use names like ~e5.0001 or ~df394b.tmp ù games sometimes call LoadLibrary with system DLL names under that folder (file not there). */
...
static HMODULE SafeDisc_TryLoadLocaleDll(const char* baseFileName)
{
	...
	if (cd && cd[0])
	{
		if (_snprintf_s(path, MAX_PATH, _TRUNCATE, "%c:\\%s", toupper((unsigned char)cd[0]), baseFileName) <= 0)
			return NULL;
		mod = LoadLibraryA_Orig(path);
		if (mod)
			log("SafeDisc: locale DLL %s -> %s (%08X)\n", baseFileName, path, (DWORD)mod);
...
BOOL WINAPI GetMessageA_Hook_Old(LPMSG lpMsg, HWND hWnd, UINT wMsgFilterMin, UINT wMsgFilterMax)
{
	...
	if (result == WAIT_TIMEOUT) {
		...
		lpMsg->pt.x = 0;
		lpMsg->pt.y = 0;
		return TRUE; // ùmessage receivedù
	}
...
BOOL WINAPI GetMessageA_Hook(LPMSG lpMsg, HWND hWnd, UINT wMsgFilterMin, UINT wMsgFilterMax)
{
	while (true)
	{
		...
		if (result == WAIT_TIMEOUT)
		{
			// Timeout expired ù return FALSE or inject behavior
...
UINT WINAPI WinExec_Hook(LPCSTR lpCmdLine, UINT uCmdShow)
{
	...
	if (ok) {
		...
		return 33; // arbitrary ùsuccessù code
	}
	else {
		return 0; // failure
	}
...
void SecuROMLoader(HMODULE hModule) 
{
	...
	if (config.GetBool("logging"))
	{
		...
		CreateConsole();
		HideConsoleCursor();
		NString logFile = config.GetValue("logFile", nullptr);
...
*/
	...
	{
		log("[Virusek] PROBE (VirusekProbeAtDllLoad): RunVirusekMethod at DLL init ù layout may be absent or encrypted here.\n");
